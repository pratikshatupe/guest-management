...
import {
  MOCK_SERVICES, MOCK_APPOINTMENTS,
} from '../../data/mockData';
import { addAuditLog } from '../../utils/auditLogger';

/**
 * DeleteServiceModal — destructive confirmation for a Service.
 *
 * Mirror of Offices/Rooms delete modals:
 *   • Named modal, not browser alert.
 *   • Cascade banner: "N appointment(s) reference this service."
 *     Reads from appointments[].servicesPrebooked — currently 0
 *     for every row since the Appointments module is not yet
 *     shipped; will populate automatically when it lands.
 *   • Typed confirmation — the Delete button stays disabled until
 *     the operator types the exact service name.
 *   • Initial focus lands on Cancel.
 *   • 800ms server-simulate before firing the success toast.
 *   • Audit log carries a full pre-delete snapshot.
 */

export default function DeleteServiceModal({ open, service, onClose, onDeleted, currentUser }) {
  const [, , , removeService] = useCollection(STORAGE_KEYS.SERVICES, MOCK_SERVICES);
  ...
  /* Cascade — number of appointments whose `servicesPrebooked`
     array contains this service id. Will start returning non-zero
     values once the Appointments module ships. */
  const cascadeCount = useMemo(() => {
    if (!service) return 0;
    return (appointments || []).filter((a) => {
      const list = Array.isArray(a?.servicesPrebooked) ? a.servicesPrebooked : [];
      return list.includes(service.id) || list.includes(service.code);
    }).length;
  }, [appointments, service]);

  if (!open || !service) return null;

  const expected = service.name;
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      addAuditLog({
        ...
        action:      'DELETE',
        module:      'Services',
        description: `Deleted service ${service.name} (${service.code}). Snapshot: ${JSON.stringify({
          id: service.id, orgId: service.orgId, category: service.category,
          chargeable: service.chargeable, price: service.price, priceUnit: service.priceUnit,
          availableOfficeIds: service.availableOfficeIds, status: service.status,
        })}`,
        orgId:       service.orgId,
      });
      setDel(false);
    ...
    >
      <div className="w-full max-w-md rounded-[14px] border border-slate-200 bg-white shadow-2xl dark:border-[#142535] dark:bg-[#0A1828]">
        ...
        <div className="flex items-start justify-between gap-3 border-b border-red-200 bg-red-50 px-5 py-4 dark:border-red-500/30 dark:bg-red-500/10">
          <div className="flex items-start gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h3 id="delete-service-title" className="font-[Outfit,sans-serif] text-[16px] font-extrabold text-red-800 dark:text-red-200">
                Delete Service
              </h3>
        ...
        <div className="px-5 py-4">
          <p id="delete-service-desc" className="text-[13px] text-slate-700 dark:text-slate-200">
          ...
          <div className="mt-4 rounded-[10px] border border-amber-200 bg-amber-50 px-3 py-2 dark:border-amber-500/30 dark:bg-amber-500/10">
            <p className="text-[12px] font-bold text-amber-800 dark:text-amber-200">
              {cascadeCount.toLocaleString('en-GB')} appointment{cascadeCount === 1 ? '' : 's'} reference this service.
            </p>
            {cascadeCount > 0 && (
              <p className="mt-1 text-[11px] text-amber-700 dark:text-amber-300">
                Remove this service from those appointments before deletion to avoid orphaned references.
              </p>
            )}
          </div>

          <label htmlFor="delete-service-confirm" className="mt-4 mb-1.5 block text-[11px] font-bold uppercase tracking-[0.06em] text-slate-500 dark:text-slate-400">
          ...
          {typed.length > 0 && !matches && (
            <p role="alert" className="mt-1 text-[11px] font-semibold text-red-500">
              Service name does not match — check for typos.
            </p>
          )}
          {error && (
        ...
        <div className="flex justify-end gap-2 border-t border-slate-200 bg-slate-50 px-5 py-4 dark:border-[#142535] dark:bg-[#071220]">
          ...
          <button
            ...
            title={matches ? `Delete ${service.name}` : 'Type the service name to enable'}
