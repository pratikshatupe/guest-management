...
const ROLES = [
  ...
  { id:'director',   label:'Director',    name:'Arjun Mehta', email:'director@corpgms.com',   password:'123456', icon:'👑', color:'#5a4bd1', bg:'#eef2f9', border:'#e9e4ff', badge:'Executive',  desc:'Organisation owner',     staffId:'staff-1', organisationId:'org-acme', officeId:'all' },
...
export default function Login({ onBackToLanding, onLogin }) {
  ...
  useEffect(() => {
    // Post-registration prefill — email+password auto-filled from registration form
    try {
      ...
      if (prefill?.email && prefill?.password) {
        ...
        sessionStorage.removeItem('cgms_reg_prefill');
        return;
      }
    } catch(e) {}
    ...
    <div style={{ minHeight:'100vh', display:'flex', fontFamily:"'Plus Jakarta Sans',sans-serif", overflow:'hidden', position:'relative' }}>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        ...
        @keyframes fillPop{0%{transform:scale(1)}40%{transform:scale(.96);background:#eef2f9}100%{transform:scale(1)}}
        ...
        .qfill-btn{display:flex;align-items:center;gap:10px;padding:10px 14px;border-radius:11px;border:1.5px solid #e9e4ff;background:#f4f7fc;cursor:pointer;font-family:inherit;transition:all .18s;text-align:left;width:100%;}
        .qfill-btn:hover{border-color:#6c5ce7;background:#eef2f9;transform:translateX(3px);}
        ...
        .eye-btn{position:absolute;right:12px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;font-size:15px;opacity:.5;padding:4px;color:#0f172a;transition:opacity .2s;}
      ...
      <div className="left-panel" style={{ width:'44%', background:'linear-gradient(135deg,#0a0f2c 0%,#1a1f4e 40%,#2d1b69 70%,#1a0a3e 100%)', display:'flex', flexDirection:'column', justifyContent:'center', padding:'clamp(44px,6vw,84px)', position:'relative', overflow:'hidden' }}>
        ...
        {PARTICLES.map((p,i)=>(
          <div key={i} style={{ position:'absolute', width:3, height:3, borderRadius:'50%', top:`${p.y}%`, left:`${p.x}%`, background:'rgba(162,155,254,.65)', animation:`particlePulse ${2+p.d}s ease-in-out ${p.d}s infinite alternate`, pointerEvents:'none' }} />
        ))}
        ...
        <div style={{ maxWidth:440, width:'100%', margin:'0 auto' }}>
          ...
          <form onSubmit={handleSubmit} noValidate>
            ...
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:26 }}>
              ...
              <a href="#" onClick={e=>e.preventDefault()} style={{ fontSize:13, color:'#6c5ce7', fontWeight:600, textDecoration:'none' }}
          ...
          <div style={{ marginTop:28 }}>
            <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:14 }}>
              <div style={{ flex:1, height:1, background:'#e9e4ff' }} />
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:7 }}>
              {ROLES.map(r=>(
                <button key={r.id} className={`qfill-btn${fillAnim===r.id?' active-fill':''}`} onClick={()=>quickFill(r)} type="button">
                  ...
                  <span style={{ fontSize:11, color:'#c4b8ff', fontWeight:600, flexShrink:0 }}>Fill →</span>
                </button>
              ))}
            </div>
            <p style={{ textAlign:'center', fontSize:11, color:'#c4b8ff', marginTop:10 }}>Click any role to auto-fill credentials, then press Log In.</p>
          </div>

          <p style={{ textAlign:'center', fontSize:12, color:'#c4b8ff', marginTop:24 }}>© 2025 CorpGMS by BIZZFLY · All rights reserved.</p>
        </div>
      </div>
    </div>
  );
